...
export function calculateForecastMonth(params: {
  ...
  const projection = calculateMonthlyFinancialProjection({
    ...params,
    knownRows: params.knownRows.filter((row) => options.sources[forecastSource(row)]),
    trackedCategories: [],
    historicalAverageByCategoryId: new Map(),
    variableProjectionItems: estimates
  });
  const sources = (['fixed', 'other', 'cards', 'variable'] as const).map((key) => {
    const rows = params.knownRows.filter((row) => forecastSource(row) === key);
    const income = rows
