...
export function usePermissions() {
  ...
  const hasAppPermission = (_perm: 'FINANCIAL_ACCOUNTS' | 'FINANCIAL_CATEGORIES'): boolean => {
    // financial permissions removed; fallback to role only
  ...
  const canAccessFinancialReports = (): boolean => false;
  const canCreateTransactions = (): boolean => false;
  const canManageFinancialAccounts = (): boolean => false;
  const canManageCategories = (): boolean => false;
