...
export function BankMatchConfidence({ confidence }: { confidence?: BankCandidate['confidence'] }) {
  ...
  const label = { HIGH: 'Alta', MEDIUM: 'Média', LOW: 'Baixa' }[confidence.level];
  const color = { HIGH: 'border-emerald-700 text-emerald-300', MEDIUM: 'border-amber-700 text-amber-300', LOW: 'border-gray-600 text-gray-300' }[confidence.level];
