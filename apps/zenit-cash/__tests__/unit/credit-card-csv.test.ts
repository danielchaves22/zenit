...
function buildReconciliationCsvInput(): Parameters<
  typeof buildCreditCardReconciliationCsv
>[0] {
  return {
    cardName: 'Nubank',
...
describe('buildCreditCardReconciliationCsv', () => {
  it('exports preview rows with current draft and match data', () => {
    const csv = buildCreditCardReconciliationCsv(buildReconciliationCsvInput());
  ...
  it('exports a confirmed existing item with its operational status', () => {
    const input = buildReconciliationCsvInput();
    const confirmedInput: Parameters<typeof buildCreditCardReconciliationCsv>[0] = {
      ...input,
      statusFilterLabel: 'OK',
      preview: {
        ...input.preview,
        summary: {
    ...
    const csv = buildCreditCardReconciliationCsv(confirmedInput);

    expect(csv).toContain('Fonte;CSV Nubank');
    expect(csv).toContain('Filtro;OK');
    expect(csv).toContain('Fatura_alvo;06/2026 (Aberta)');
    expect(csv).toContain('Ja_conciliados;1');
    expect(csv).toContain('Pendentes;0');
    expect(csv).toContain('Importaveis;0');
    expect(csv).toContain('Valor_importavel;0,00');
    expect(csv).toContain(
      'Selecionado;Item;Status;Motivo;Descricao_fatura;Descricao_a_importar;Categoria_a_importar;Categoria_sugerida;Fonte_sugestao;Valor;Valor_assinado;Data_compra;Parcela;Secao;Tipo_linha;Direcao;Final_cartao;Importavel;Correspondencias;Correspondencias_detalhes;Motivo_nao_importavel;Item_id'
    );
    expect(csv).toContain(
      'Nao;1;OK;Lancamento existente confirmado nesta conciliacao.;"Padaria; Centro";Padaria Centro;Alimentacao;Alimentacao;Regra;50,00;-50,00;02/06/2026;1/2;Compras;PURCHASE;Debito;1234;Nao;1;Lancamento | Padaria Centro | 50,00 | 02/06/2026 | 1/2 | fatura 06/2026 | id 22;;item-1'
    );
  });
});
